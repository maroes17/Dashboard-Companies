...
export function NewVehicleDialog({ open, onOpenChange, onSave }: NewVehicleDialogProps) {
  ...
  useEffect(() => {
    if (formData.tipo) {
      ...
      // Resetear marca y modelo si el tipo cambia
      if (!marcas.includes(formData.marca)) {
        setFormData(prev => ({ 
          ...prev, 
          marca: "", 
          modelo: "" 
  ...
  useEffect(() => {
    if (formData.marca) {
      ...
      // Resetear modelo si la marca cambia y el modelo actual no está disponible
      if (!modelos.includes(formData.modelo)) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // Si se ha enviado el formulario previamente, validar en tiempo real
  ...
  const validateField = (name: string, value: string) => {
    ...
    if (["patente", "marca", "modelo", "tipo", "estado"].includes(name)) {
      if (!value.trim()) {
        newErrors[name] = `El campo ${name.replace('_', ' ')} es obligatorio`;
      } else {
    ...
    if (["anio", "km_actual", "km_ultimo_servicio", "km_proximo_servicio", "consumo_promedio"].includes(name)) {
      if (value.trim() && isNaN(Number(value))) {
        newErrors[name] = `El campo ${name.replace('_', ' ')} debe ser un número`;
      } else {
        delete newErrors[name];
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newVehicle: Fleet = {
      id_flota: Date.now(), // Temporal, Supabase lo asignará
      ...
      estado: formData.estado as 'activo' | 'inactivo' | 'mantenimiento' | 'en_reparacion' | 'dado_de_baja',
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder="Km último servicio"
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder="Km próximo servicio"
            ...
            <div className="grid grid-cols-1 gap-2 mt-4">
              ...
              <Textarea
                id="observaciones"
                name="observaciones"
                placeholder="Información adicional relevante sobre el vehículo"
